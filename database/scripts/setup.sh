...
psql -U "${POSTGRES_USER}" -d "${POSTGRES_APP_DB}" -c "GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO ${POSTGRES_APP_USER};"
psql -U "${POSTGRES_USER}" -d "${POSTGRES_APP_DB}" -c "GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO ${POSTGRES_APP_USER};"
psql -U "${POSTGRES_USER}" -d "${POSTGRES_APP_DB}" -c "ALTER SCHEMA public OWNER TO ${POSTGRES_APP_USER};"
psql -U "${POSTGRES_USER}" -d "${POSTGRES_APP_DB}" -c "GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA public TO ${POSTGRES_APP_USER};"
psql -U "${POSTGRES_USER}" -d "${POSTGRES_APP_DB}" -c "GRANT ALL PRIVILEGES ON ALL PROCEDURES IN SCHEMA public TO ${POSTGRES_APP_USER};"
